...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/wyutingHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    priority = StringField("What is the new priority of the task?", validators=[Required()])
    submit = SubmitField('Update')
...
def update(item):
    ...
    if form.validate_on_submit():
        new_priority = form.priority.data
        task = db.session.query(TodoItem).filter_by(id=int(item)).first()
        task.priority = new_priority
        db.session.commit()
        flash("Updated priority of " + task.description)
        return redirect(url_for('all_lists'))
    return render_template('update_item.html',task = item, form = form)
...
def delete(lst):
    ...
    print(type(lst))
    form = DeleteButtonForm()
    task_list = db.session.query(TodoList).filter_by(id=int(lst)).first()
